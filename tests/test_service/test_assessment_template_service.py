...
def test_get_template_by_inexistent_id(db_session: Session):
    with pytest.raises(HTTPException):
        AssessmentTemplateService(db_session).get_template_by_id(1) 
...
def test_create_template_success(db_session: Session, mock_subject: Subject):
...
def test_get_template_by_id_success(db_session: Session, mock_assessment_template_semester1: AssessmentTemplate):
    service = AssessmentTemplateService(db_session)
    template = service.get_template_by_id(mock_assessment_template_semester1.id)
    assert template.name == "Math Test"

def test_update_template_success(db_session: Session, mock_assessment_template_semester1: AssessmentTemplate):
    service = AssessmentTemplateService(db_session)
...
def test_delete_template_success(db_session: Session, mock_subject: Subject):
    service = AssessmentTemplateService(db_session)
