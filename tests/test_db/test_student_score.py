...
def test_student_score_validation_all_cases(db_session: Session, mock_student: Student, mock_assessment_template: AssessmentTemplate):
    # invalid score 
    with pytest.raises(ValueError):
        StudentScore(student_id= mock_student.id, assessment_template_id= mock_assessment_template.id, score= -1)  # Điểm âm
    #invalid status 
    with pytest.raises(ValueError):
        StudentScore(student_id= mock_student.id,
                      assessment_template_id= mock_assessment_template.id,
...
def test_hacker_insert_invalid_student_score(db_session: Session, mock_student: Student, mock_assessment_template: AssessmentTemplate):
    ...
    data= {
        ...
        'template_id': str(mock_assessment_template.id),
